...
class ConfigLoader:
    ...
    def _load_single_config(self, config_file: Path) -> DatasetConfig:
        ...
        required_fields = [
            'name', 'description', 'function_name', 'requires_year',
            'data_type', 'partition_by', 'validation', 'extraction', 'output'
        ]
    ...
    def _validate_config_data(self, config_data: Dict[str, Any], config_file: Path) -> None:
        ...
        if 'required_columns' not in validation:
            raise ValueError(f"Missing 'required_columns' in validation section of {config_file}")
        ...
        if 'timeout_seconds' not in extraction:
            raise ValueError(f"Missing 'timeout_seconds' in extraction section of {config_file}")
    ...
    def get_output_path(self, dataset_name: str, year: Optional[int] = None, 
                       etl_date: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            raise ValueError(f"Missing template variable {e} for dataset {dataset_name}")