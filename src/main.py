...
def main(args: argparse.Namespace) -> None:
    ...
    # conn = duckdb.connect("md:nfl_py")
    token_md = os.getenv(args.md_token)
    print(token_md)
    db_md = args.md_db
    print(db_md)
    url = f"duckdb:///md:{db_md}?motherduck_token={token_md}"
    print(url)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Token for motherduck, set in .profile, .zprofile or local",
    )
    parser.add_argument(
        "--md_db",
        default="nfl_py",
        help="DB for connecting to motherduck",
    )
    parser.add_argument(
        "--years_beg",
        default=1999,
