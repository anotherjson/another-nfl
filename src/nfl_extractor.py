...
class NFLDataExtractor:
    ...
    def extract_dataset(
        ...
        self.logger.info(
            f"Starting extraction for {dataset_name}, year={year}, etl_date={etl_date}"
        )
        ...
        try:
            ...
            if validate and not data.empty:
                ...
                if not validation_result["passed"]:
                    self.logger.warning(
                        f"Data validation failed for {dataset_name}: {validation_result}"
                    )
            else:
                extraction_metadata["validation_passed"] = (
                    True  # Skip validation or empty data
                )
    ...
    def _extract_with_retry(
        self, config: DatasetConfig, year: int | None = None
    ) -> pd.DataFrame:
        """
        ...
        for attempt in range(retry_attempts):
            try:
                self.logger.info(
                    f"Extraction attempt {attempt + 1}/{retry_attempts} for {config.name}"
                )
    ...
    def _validate_data(
        self, data: pd.DataFrame, config: DatasetConfig
    ) -> dict[str, Any]:
        """
    ...
    def extract_multiple_years(
        ...
        if not config.requires_year:
            raise ValueError(
                f"Dataset '{dataset_name}' doesn't support year-based extraction"
            )
        ...
        for i, year in enumerate(years, 1):
            self.logger.info(
                f"Extracting year {year} ({i}/{total_years}) for {dataset_name}"
            )
            ...
            try:
                ...
                self.logger.error(
                    f"Failed to extract {dataset_name} for year {year}: {str(e)}"
                )
    ...
    def get_extraction_status(
        self, dataset_name: str, year: int | None = None
    ) -> dict[str, Any]:
        """
