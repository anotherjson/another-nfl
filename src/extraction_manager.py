...
class ExtractionManager:
    ...
    def _load_state(self) -> dict[str, Any]:
        ...
        if self.state_file.exists():
            try:
                ...
                self.logger.warning(
                    f"Failed to load state file: {e}. Starting with empty state."
                )
    ...
    def get_missing_extractions(self, dataset_name: str, years: list[int]) -> list[int]:
        ...
        extracted_years = set(
            int(year)
            for year in self.state["extractions"][dataset_name].keys()
            if self.state["extractions"][dataset_name][year].get("status")
            == "completed"
        )
    ...
    def is_extraction_current(
        self, dataset_name: str, year: int | None = None, max_age_days: int = 1
    ) -> bool:
        """
    ...
    def extract_incremental(
        ...
        try:
            if config.requires_year:
                if not years:
                    raise ValueError(
                        f"Dataset '{dataset_name}' requires years to be specified"
                    )
                ...
                if not force_refresh:
                    years_to_extract = [
                        year
                        for year in years
                        if not self.is_extraction_current(
                            dataset_name, year, max_age_days
                        )
                    ]
                ...
                for year in years_to_extract:
                    try:
                        ...
                        self._record_extraction(
                            dataset_name, year, metadata, "completed"
                        )
                        ...
                        self._record_extraction(
                            dataset_name, year, {"error": str(e)}, "failed"
                        )
                        self.logger.error(
                            f"Failed to extract {dataset_name} year {year}: {e}"
                        )

            else:
                # Non-year dataset
                if not force_refresh and self.is_extraction_current(
                    dataset_name, None, max_age_days
                ):
                    ...
                    try:
                        data, metadata = self.extractor.extract_dataset(
                            dataset_name=dataset_name, validate=True, save_to_disk=True
                        )

                        # Update state
                        self._record_extraction(
                            dataset_name, None, metadata, "completed"
                        )
                        ...
                        extraction_summary["errors"].append(error_msg)
                        self._record_extraction(
                            dataset_name, None, {"error": str(e)}, "failed"
                        )
    ...
    def get_extraction_summary(
        ...
        if dataset_name:
            if dataset_name not in self.state["extractions"]:
                return {
                    "dataset": dataset_name,
                    "extractions": {},
                    "summary": "No extractions recorded",
                }
        ...
        for ds_name in datasets_to_process:
            ...
            for year_key, extraction_info in ds_extractions.items():
                ...
                if (
                    not summary["last_extraction"]
                    or extraction_date > summary["last_extraction"]
                ):
                    summary["last_extraction"] = extraction_date
                if (
                    not ds_summary["last_extraction"]
                    or extraction_date > ds_summary["last_extraction"]
                ):
    ...
    def cleanup_old_extractions(self, max_age_days: int = 30) -> dict[str, Any]:
        ...
        empty_datasets = [
            ds
            for ds, extractions in self.state["extractions"].items()
            if not extractions
        ]
