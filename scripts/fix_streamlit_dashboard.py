#!/usr/bin/env python3
"""
Fix Streamlit dashboard to work with available data
"""
...
def create_simple_dashboard():
    """Create a working dashboard using available NFL data"""
    
    # Read available team data directly from parquet
...
def update_streamlit_main():
    """Update main Streamlit file to use available data"""
...
"""
    ...
    streamlit_path = Path("visualizations/streamlit_app/main.py")
    with open(streamlit_path, 'w') as f:
        f.write(streamlit_main)
    
    print(f"✅ Updated Streamlit dashboard: {streamlit_path}")

if __name__ == "__main__":
    # Test data loading
    df = create_simple_dashboard()
    
    if df is not None:
        # Update the dashboard
        update_streamlit_main()
        print("✅ Dashboard updated to use available data")
    else:
